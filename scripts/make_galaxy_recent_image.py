...
if __name__ == "__main__":
    ...
    sql = "SELECT `x`, `y`, `z`, `last_updated` FROM `systems` WHERE last_updated >= %s"
    cursor.execute(sql, (base_time,))
    while True:
        row = cursor.fetchone()
        if row is None:
            break
        ...
        val = int(((last_updated - base_time) / (now - base_time)) * 255)
    ...
    draw.text((0, FONT_SIZE * 0), "Recently updated systems map", font=font)
    ...
    final_image.save("galaxy_recent.png")