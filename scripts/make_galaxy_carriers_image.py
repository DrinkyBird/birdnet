...
if __name__ == "__main__":
    ...
    now = int(time.time())
    base_time = now - 86400 * 2

    sql = "SELECT `systems`.`x`, `systems`.`y`, `systems`.`z`, `systems`.`id`, `markets`.`system_id` FROM `markets`, `systems` WHERE `markets`.`market_type` = 'FleetCarrier' AND `systems`.`id` = `markets`.`system_id`"
    cursor.execute(sql)
    while True:
        row = cursor.fetchone()
        if row is None:
        ...
        total_carriers += 1
        if row['system_id'] not in seen_systems:
            seen_systems.append(row['system_id'])
    ...
    draw.text((0, FONT_SIZE * 0), "Map of known Fleet Carriers", font=font)
    draw.text((0, FONT_SIZE * 1), f"{total_carriers:,} carriers in {len(seen_systems):,} systems", font=font)
    ...
    final_image.save("galaxy_carriers.png")